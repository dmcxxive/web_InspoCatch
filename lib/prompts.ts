...
【时效与事实边界（须遵守）】
8. 以本任务所给热点材料为事实下界：材料未写明、无法交叉核实的信息，不得写成既定事实；禁止臆造「已发布、已上市、已开售、已实测、已拿到真机、已参与内测」等情节。
9. 开箱、评测、上手、体验类表述：仅当材料能明确支撑「产品/服务已对外可用且信息可核」时才允许写实测口吻。若材料仅为预告、爆料、传闻或产品尚未正式发布，须改为「前瞻」「期待」「根据公开信息梳理」「待官方确认」等，并显式说明不确定性，不得写成已完成的开箱或深度评测。
10. 涉及具体上市时间、价格、参数、销量、渠道等：无材料依据则不得编造；可写「据××来源」「若后续官宣为准」，避免替读者下结论。
11. 严格区分事实、观点与传闻：传闻、爆料须标明性质，不得写成亲历或定论。

【成稿形态：微信公众号后台正文（须遵守）】
12. 要求作者输出的正文须为可直接粘贴进微信公众号后台图文编辑区「正文」的纯文本式层级排版；编辑将依赖后台工具栏完成加粗、字号、颜色、对齐等，故成稿中禁止使用 Markdown 或其它标记语法（包括但不限于 # 标题、**粗体**、行首 - 列表、[文字](链接)、![图](url)、反引号代码块、HTML 标签等）。
13. 结构建议：段与段之间空一行；小标题单独占一行，可用「一、」「二、」或【小标题】等；列举用「1.」「2.」或「（1）（2）」等中文习惯，勿用 Markdown 列表符号。
14. 需要强调处：可拆短句单独成段，或句末附简短提示如「（此句建议后台加粗）」；勿用 ** 等符号表示加粗。
15. 若需插图位置：在对应段落后单独一行写「【配图说明：……】」或「（此处插入××主题配图）」，便于人工插入；禁止使用 Markdown 图片语法。
...
- 在上述两节之后，再写具体写作任务与结构建议；写作任务中必须显式写入上述「时效与事实边界」与「成稿形态：微信公众号后台正文」的要求，不得省略。
`.trim();
...
你的任务是撰写一段可直接粘贴给内容生成 AI 的「长文写作系统指令」（中文纯文本，不要用 JSON）。该指令将用于生成最终可发到微信公众平台的稿件，因此必须强调「公众号后台可粘贴的正文排版」与「事实边界」，而非 Markdown 技术稿。
...
3. 正文任务部分：目标读者、语气、结构（引言-论证-对照-收尾）；必须单列「事实与时效」要求：紧扣热点材料，禁止编造未发布产品的开箱/实测，禁止把传闻写成本人亲历或定论；无依据的时间、价格、参数不得杜撰。
4. 正文任务部分必须单列「成稿输出形态」：最终正文须为可粘贴进微信公众号后台正文编辑区的纯文本层级排版（段间空行、中文小标题与序号列举），明确禁止使用 Markdown/HTML 标记；并说明加粗等格式由编辑在后台手动设置，成稿仅用文字提示（如「（此段建议加粗）」）如需。
5. 语气与风格：必须严格贯彻 toneAxisGuidance；当它与 writingVoiceGuidelines 中的条目冲突时，以 toneAxisGuidance 为准。
6. 配图：若 imageSearchPromptGuidance 要求加入配图说明，你须在输出的写作指令中约定：在正文对应位置单独一行写「【配图说明：……】」或「（此处插入××主题配图，搜图关键词：……）」等纯文字提示，便于人工搜图插入；禁止使用 Markdown 图片语法或 HTML 注释。若 imageSearchPromptGuidance 说明用户未要求，则不要在写作指令中加入任何配图占位。
7. 整体须落实 writingVoiceGuidelines 中不与 toneAxisGuidance 冲突的反套作、强化新闻信息、批判力度、时效与事实边界、公众号正文形态等要求，不得写成中立百科或软文软广。
8. 若 toneBlend 较低（偏学术），须在正文任务中明确要求：成稿禁止使用比喻、类比作主要论证方式；并禁止用双引号词语来暗示讽刺性指称。若 toneBlend 较高（偏随意），可弱化本条中关于比喻的硬性禁止，但仍须避免空洞套话，且不得放松对事实边界的约束。
...
无论偏向哪一端，都必须保留 persona 与立场的一致性，不得为了「随意」而编造与人设矛盾的态度；且不得放松对热点材料与可核实信息的边界（未证实的产品状态不得写成已发布或已实测）。
`.trim();
}

export function buildImageSearchPromptGuidance(includeImagePromptHints: boolean): string {
  if (includeImagePromptHints) {
    return `用户要求：在输出的「长文写作系统指令」中，必须约定下游成稿如何在公众号正文中用纯文字标注配图（例如单独一行「【配图说明：……，搜图关键词：……】」或「（此处插入××主题配图）」），便于人工或后续步骤搜图插入；禁止使用 Markdown 图片语法。请说明插入频率与注意事项。`;
  }
